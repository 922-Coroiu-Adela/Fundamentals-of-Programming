...
class TextFileRepository:
    ...
    def load_data(self):
        ...
        for line in file:
            line = line.strip()
            line = line.split(" ")
            aux.append(ComplexNumber(int(line[0]), int(line[1])))
        if len(aux) < 10:
    ...
    def save_changes(self):
        ...
        for complex_number in self.complex_numbers:
            file.write(str(complex_number.get_real()) + " " + str(complex_number.get_imaginary()) + "\n")
        file.close()
